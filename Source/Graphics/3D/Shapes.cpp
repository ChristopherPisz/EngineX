                    ...
                    const bool flipNormals)
{
   ...
   BaseException exception("Not Set", 
                           "void GenerateSphere(ID3D10Device & device, std::vector<Buffer::SharedPtr> & buffers, float radius, unsigned segments)",
                           "Shapes.cpp");
   ...
   if( segments < 3 )
   {
      std::stringstream msg("Sphere requires a minimum of 3 segments to be classified as a sphere at all.");
